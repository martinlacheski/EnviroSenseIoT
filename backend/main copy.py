...
async def startup():
    ...
    # Obtener el event loop principal de FastAPI
    ...
    # Suscripción a tópicos de publicación de sensores y actuadores
    mqtt_client.subscribe("environmental/sensor/pub", lambda topic, payload: process_sensor_message_pub(topic, payload, loop))
    mqtt_client.subscribe("nutrient_solution/sensor/pub", lambda topic, payload: process_sensor_message_pub(topic, payload, loop))
    mqtt_client.subscribe("consumption/sensor/pub", lambda topic, payload: process_sensor_message_pub(topic, payload, loop))
    mqtt_client.subscribe("actuators/pub", lambda topic, payload: process_sensor_message_pub(topic, payload, loop))
