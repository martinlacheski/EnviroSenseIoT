...
% La Figura \ref{fig:flujo_nodos} presenta el diagrama de flujo de alto nivel del
% firmware de los nodos. Se detalla el proceso desde la inicialización del
% hardware hasta la ejecución cíclica del bucle principal, se incluye la gestión
% de comandos MQTT y las acciones específicas para sensores y actuadores.
...
% La transmisión de los datos de sensores y los comandos de control se realiza
% mediante el protocolo MQTT, a través de mensajes en formato JSON publicados en
% los tópicos definidos en el archivo \texttt{config.py}. Se implementó un
% esquema de mensajes estandarizado para facilitar el procesamiento en el backend
% y en los nodos.
...
% El código de la implementación del firmware de los nodos se publicó en un
% repositorio público de GitHub \cite{GithubFirmware} bajo la licencia MIT. Este
% repositorio contiene el código fuente del firmware de los nodos, así como la
% documentación necesaria para su instalación, implementación y uso.

% El firmware de los nodos fue desarrollado y publicado en un repositorio público
% de GitHub \cite{GithubFirmware} bajo licencia MIT. En dicho repositorio se
% encuentran disponibles tanto el código fuente como la documentación requerida
% para su instalación, implementación y utilización.
