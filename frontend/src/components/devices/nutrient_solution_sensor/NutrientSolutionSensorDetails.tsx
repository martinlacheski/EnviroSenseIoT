...
import NutrientSolutionSensorModalForm from './NutrientSolutionSensorModalForm';
import { deleteNutrientSolutionSensor, getEnvironments } from '@/api/index';


type NutrientSolutionSensorDetailsProps = { sensor: NutrientSolutionSensor };

export default function NutrientSolutionSensorDetails({ sensor }: NutrientSolutionSensorDetailsProps) {
    ...
    const { mutate: deleteMutation } = useMutation({
        mutationFn: deleteNutrientSolutionSensor,
        ...
        onSuccess: (data: { message: string }) => {
            ...
            queryClient.invalidateQueries({ queryKey: ['nutrient_solution_sensors'] }); // Actualizar la lista
        },
    });
    ...
    return (
        <>
            ...
            <ConfirmationDialog
                ...
                title="Eliminar Sensor de solución nutritiva"
                message={`¿Estás seguro de eliminar el Sensor de solución nutritiva "${sensor.description}"?`}
            />

            <NutrientSolutionSensorModalForm open={isEditModalOpen} setOpen={setIsEditModalOpen} sensor={sensor} />
        </>
    );
}