...
import { deleteConsumptionSensor, getEnvironments } from '@/api/index';
import ConsumptionSensorModalForm from './ConsumptionSensorModalForm';

type ConsumptionSensorDetailsProps = { sensor: ConsumptionSensor };

export default function ConsumptionSensorDetails({ sensor }: ConsumptionSensorDetailsProps) {
    ...
    const { mutate: deleteMutation } = useMutation({
        mutationFn: deleteConsumptionSensor,
        ...
        onSuccess: (data: { message: string }) => {
            ...
            queryClient.invalidateQueries({ queryKey: ['consumption_sensors'] }); // Actualizar la lista
        },
    });
    ...
    return (
        <>
            ...
            <ConfirmationDialog
                ...
                title="Eliminar Sensor de Consumos"
                message={`¿Estás seguro de eliminar el Sensor de Consumos "${sensor.description}"?`}
            />

            <ConsumptionSensorModalForm open={isEditModalOpen} setOpen={setIsEditModalOpen} sensor={sensor} />
        </>
    );
}